...
class User {
        ...
        public function __construct(PDO $objPDO, $id = NULL) {
                ...
                if (isset($id)) {
                        ...
                        $strQuery .= " FROM public." . $this->strTableName . " WHERE
                                     \"id\" = :eid";
                        ...
                        $log->logMessage('Found user with Id '.$this->ID, Logger::INFO);
                };
        }
        ...
        public function Save() {
                if (isset($this->ID)) {
                        ...
                        $objStatement->execute();
                        $log = Logger::getInstance();
                        $log->logMessage('Updating new User with Id '.$this->ID, Logger::INFO);
                        ...
                        $log = Logger::getInstance();
                        $log->logMessage('Creating new User with Id '.$this->ID, Logger::INFO);
        ...
        public function MarkForDeletion() {
                $log = Logger::getInstance();
                $log->logMessage('Marked user '.$this->ID.' for deletion', Logger::DEBUG);
                $log->logMessage('This action cannot be reverted after unset is triggered', Logger::WARNING);
        ...
        private function SetAccessor($strMember, $strNewValue) {
                if (property_exists($this, $strMember)) {
                        ...
                        $log = Logger::getInstance();
                        $log->logMessage('Property '.$strMember.' is set as '.$strNewValue, Logger::DEBUG);

                } else {
                        $log = Logger::getInstance();
                        $log->logMessage('Property '.$strMember.' doesnt exist', Logger::DEBUG);
                        return(false);
        ...
        private function GetAccessor($strMember) {
                if (property_exists($this, $strMember)) {
                        ...
                        $log = Logger::getInstance();
                        $log->logMessage('Acceses to '.$strMember, Logger::INFO);

                        return($strRetVal);
                } else {
                        $log = Logger::getInstance();
                        $log->logMessage('Property '.$strMember.' doesnt exist', Logger::ERROR);
                        return(false);
                };
        }
        public function __ToString(){
                $log = Logger::getInstance();
                $log->logMessage('Showing User Details', Logger::INFO);
